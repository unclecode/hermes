...
from hermes.core import transcribe, Hermes, generate_video_commentary, generate_textual_commentary
...
result = generate_video_commentary(
    'examples/assets/input_football.mp4', force = True, 
    interval_type = "total_snapshots", snapshot_count=8, 
    video_topic='sports', commentary_type='exciting',
    target_size=224, video_output_size=(640,)
)
print(f"Commentary: {result['commentary'][:100]}...")
print(f"Final video path: {result['final_video_path']}\n")

# Example 8: Generate textual commentary for a local file
print("Example 8: Generate textual commentary for a local file")
# result = hermes.generate_textual_commentary('examples/assets/input_football.mp4')
result = generate_textual_commentary(
        'examples/assets/input_football.mp4', force = True, 
        interval_type = "seconds", interval_value=10, 
        video_topic='sports', commentary_type='technical and detailed',
        target_size=224, video_output_size=(640,))
